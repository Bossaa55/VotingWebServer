...
const ParticipantCard = ({ participant }: { participant: Participant;}) => {
    return(
        <Card className="shadow-md w-96 z-10 py-2">
            <CardContent className="flex items-center px-4 gap-4 h-24">
                <img
                    src={participant.imageUrl}
                    alt="Contestant"
                    className="size-16 object-cover rounded-full aspect-square"
                />
                <div className="flex justify-between w-full">
                    <span className="text-xl">{participant.name}</span>
                    <a href={`/admin/manage/edit?p=${participant.id}`} className="cursor-pointer text-gray-500 hover:text-gray-700">
...
export const AdminManagePage = () => {
    ...
    const [participants, setParticipants] = useState<Array<Participant> | null>(null);
